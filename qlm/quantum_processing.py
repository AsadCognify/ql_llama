...
class LLAMA3:
    ...
    def finetune(params: Dict[str, Any], ft: str = None):
        try:
            ...
            training_dataset = load_dataset_for_training(data_path=config.training_data_path.replace(".csv", ".json"), batch_size=config.batch_size, save_steps=config.save_steps)
            LLAMA3.send_log_to_flask(message=f"\n\nFROM THE WORKER: Loading dataset from {config.validation_data_path.replace('.csv', '.json')}")
            validation_dataset = load_dataset_for_validation(data_path=config.validation_data_path.replace(".csv", ".json"), batch_size=config.batch_size, save_steps=config.save_steps)
            LLAMA3.send_log_to_flask(message=f"\n\nFROM THE WORKER: Loading datasets complete!")


            LLAMA3.send_log_to_flask(message=f"\n\nFROM THE WORKER: Evaluating training dataset\nLength of training dataset: {len(training_dataset)}\n{training_dataset}")
            # Calculate save steps
            # if config.save_steps >= int( len(training_dataset) / config.batch_size ):

            #     config.save_steps = int( len(training_dataset) / config.batch_size ) - 5
